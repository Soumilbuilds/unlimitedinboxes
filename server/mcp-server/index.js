...
const INTERNAL = 'http://127.0.0.1:3000/api';
const app = express();
app.use(cors());
...
function createServer(apiKey) {
	...
	s.setRequestHandler(z.object({ method: z.literal('tools/list') }), async () => ({
		tools: [
			...
			{ name: 'list_orders', description: 'List all orders. Optionally filter by domain.', inputSchema: { type: 'object', properties: { domain: { type: 'string' } } } },
			{
				...
				inputSchema: {
					type: 'object',
					required: ['tenant_domain', 'admin_email', 'admin_password', 'mfa_secret', 'mailbox_password'],
					properties: {
						tenant_domain: { type: 'string' },
						admin_email: { type: 'string' },
						admin_password: { type: 'string' },
						mfa_secret: { type: 'string' },
						total_mailboxes: { type: 'integer' },
			...
			{ name: 'start_order', description: 'Start provisioning mailboxes for an order.', inputSchema: { type: 'object', required: ['order_id'], properties: { order_id: { type: 'integer' } } } },
	...
	s.setRequestHandler(z.object({ method: z.literal('tools/call'), params: z.object({ name: z.string(), arguments: z.record(z.any()).optional() }) }), async (req) => {
		try {
			...
			switch (name) {
				case 'get_account': result = await apiCall('/orders/stats', apiKey); break;
				case 'list_orders': result = await apiCall(`/orders${a.domain ? `/by-domain/${encodeURIComponent(a.domain)}` : ''}`, apiKey); break;
				case 'create_order': {
					const body = {
						tenant_domain: a.tenant_domain,
						admin_email: a.admin_email,
						admin_password: a.admin_password,
						mfa_secret: a.mfa_secret,
						total_mailboxes: Number(a.total_mailboxes || 100),
						mailbox_password: a.mailbox_password,
						order_name: a.order_name || '',
					};
					result = await apiCall('/orders', apiKey, { method: 'POST', body });
					break;
				}
				case 'get_order': result = await apiCall(`/orders/${a.order_id}`, apiKey); break;
				case 'start_order': result = await apiCall(`/orders/${a.order_id}/start`, apiKey, { method: 'POST' }); break;
				case 'download_csv': result = await apiCall(`/orders/${a.order_id}/download`, apiKey); break;
...
app.get('/mcp', async (req, res) => {
	...
	if (!apiKey) return res.status(400).json({ error: 'Missing ?api_key= parameter. Get one at https://app.unlimitedinboxes.com' });

	try { const check = await apiCall('/orders/stats', apiKey); if (check?.error) return res.status(401).json({ error: 'Invalid API key.' }); }
